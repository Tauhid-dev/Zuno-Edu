---
name: zuno-webhook-security
description: "Apply signed external event processing rules when the selected Zuno Edu chunk changes this boundary."
---
# Signed external event processing
...
Verify original bytes/signature/account/mode, deduplicate durable event ID and process current server state. Never rely on order or browser redirects.

## Verification requirements
Replay duplicates/out-of-order events and reject forged/expired signatures. Record actual results and evidence in the chunk handoff; planned checks are not passes.
